...
global			 $pointer_type = 0 ; set from configuration
...
global const $REGKEY_POINTER = "Pointer"
global const $REGKEY_SENSIT  = "Sensitivity"

global const $POINTER_WHITE = 1
global const $POINTER_BLACK = 2
...
global $idAbout, $idExit, $idAutoStart, $idEnabled, $idUseCtrl, $idPointer, $idPointer0, $idPointer1, $idSensit, $idSensit0, $idSensit1, $idSensit2
global $hBmp, $hIcon
...
Func _createTray ()
  ...
  _TrayAddLine()
	$idPointer0 = TrayCreateItem("White Pointer")
	$idPointer1 = TrayCreateItem("Black Pointer")
  _TrayAddLine()
  ...
  Switch TrayGetMsg()
    ...
    Case $idUseCtrl
      ...
      EndIf
		...
		case $idPointer0
			_setPointer($POINTER_WHITE)
		case $idPointer1
			_setPointer($POINTER_BLACK)
...
Func _showBig()
	If $showing = 0 Then
		$showing = 1
		If $pointer_type = 2 Then
			$g_hImage = $g_hImage_black
		Else
			$g_hImage = $g_hImage_white
		EndIf
...
Func _getPointer()
	$pval = Number(RegRead($REGKEY_CONFIG, $REGKEY_POINTER))
	If $pval < 1 Then
		$pval = 1
	EndIf
	Return $pval
EndFunc
Func _setPointer($pval)
	If $pval = 0 Then
		$pval = _getPointer()
	Else
		RegWrite($REGKEY_CONFIG, $REGKEY_POINTER, "REG_DWORD", $pval)
	EndIf

	$pointer_type = $pval

	local const $pname = _setPointerToName($pval)
	TrayItemSetState($idPointer0, _trayChecked($pname = "White"))
	TrayItemSetState($idPointer1, _trayChecked($pname = "Black"))
EndFunc
Func _setPointerToName($pval)
	If $pval = $POINTER_BLACK Then
		Return "Black"
	Else
		Return "White"
	EndIf
...
Func _Torus($pointer, $bSaveBinary = False, $sSavePath = @ScriptDir)
